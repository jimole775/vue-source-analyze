...
function initLifecycle (vm) {
  ...
  // abstract 表示一个抽象实例，抽象实例是不生成实际节点的
  if (parent && !options.abstract) {
    // 向上寻找到一个 非抽象 类型的 vm 实例
...
initMixin(Vue$3); // 初始化vue的所有属性
stateMixin(Vue$3); // 定义$watch，用于手动启用watch属性，生成Watcher实例
eventsMixin(Vue$3); // 绑定监听事件，$on, $off, $emit, $once
